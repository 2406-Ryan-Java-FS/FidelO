...
import com.revature.item_management_application.services.TrainerService;
...
public class TrainerRestController {
    ...
    public ResponseEntity<?> updateTrainer(@PathVariable int trainerId, @RequestBody Trainer theTrainer) {
        try 
        {
            ...
            Trainer updatedTrainer = trainerService.updateTrainerById(theTrainer, trainerId);
    ...
    {
        int rowsAffected = trainerService.deleteTrainerById(trainerId);
    ...
    /**
     * handler to get a list of trainers associated with a specific regional professor.
     * it handles the GET request at the "/admin_console/{regProfId}/trainers" endpoint.
     * @param regProfId the ID of the regional professor whose associated trainers are to be retrieved.
     * @return a ResponseEntity that contains a list of Trainer objects associated with the given regional professor.
     */
    @GetMapping("/admin_console/{regProfId}/trainers")
    public ResponseEntity<List<Trainer>> getMessagesByAccountId(@PathVariable Integer regProfId) {
        List<Trainer> trainers = trainerService.getTrainersByRegionalProfessorId(regProfId);
